  ...
  return (
    <div className="flex-1 my-16 w-full">
        ...
        <div className="mt-8">
            {
            boards.map((board, i) => (
                <div key={i} className={`group cursor-pointer flex space-x-3 items-center pl-6 w-11/12 transition duration-500 bg-opacity-0 bg-mainPurple dark:hover:bg-white rounded-r-full hover:bg-opacity-10 ${currentBoard.name === boards[i].name && 'active-board'}`} onClick={() => setActiveBoard(i)}>
